...
type
  TfrmCachedUpdates = class(TfrmMainQueryBase)
    ...
    SqlClientTableProducts: TSqlClientTable;
...
begin
  SqlClientTableProducts.UpdateBatch;
end;

procedure TfrmCachedUpdates.btnCancelClick(Sender: TObject);
begin
  SqlClientTableProducts.CancelBatch;
end;

procedure TfrmCachedUpdates.btnConnectClick(Sender: TObject);
begin
  inherited;
  SqlClientTableProducts.Close;
  SqlClientTableProducts.Open;
  with SqlClientTableProducts.FieldByName('ProductID') do
    ProviderFlags := [pfInWhere, pfInKey];
end;

procedure TfrmCachedUpdates.btnRevertClick(Sender: TObject);
begin
  SqlClientTableProducts.RevertRecord;
end;

procedure TfrmCachedUpdates.FormCreate(Sender: TObject);
begin
  inherited;
  RegisterDS(SqlClientTableProducts);
end;
