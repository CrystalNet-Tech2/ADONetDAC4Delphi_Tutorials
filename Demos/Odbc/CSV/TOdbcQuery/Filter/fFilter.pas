...
begin
  with OdbcQueryOrders do begin
    Filtered := False;
    OnFilterRecord := nil;
    Filter := 'Freight > 30 AND Freight < 70';
    Filtered := True;
  end;
  edtExample.Text := OdbcQueryOrders.Filter;
end;
...
begin
  statStatus.Panels[0].Text := 'Total Records: ' + VarToStr(OdbcQueryOrders.Compute('Count(OrderID)', ''));
end;
...
begin
  with DataSet.FieldByName('OrderID') do
    Accept := (AsInteger >= 10248) and (AsInteger <= 10343);
end;
