...
type
  TfrmFilter = class(TfrmMainQueryBase)
    ...
    procedure OracleClientQueryOrdersFilterRecord(DataSet: TDataSet;
      var Accept: Boolean);
...
begin
  with OracleClientQueryOrders do
  begin
    Filtered := False;
    Filter := '';
    Filtered := True;
...
begin
  ...
  edtExample.Text := OracleClientQueryOrders.Filter;
end;
...
begin
  with OracleClientQueryOrders do begin
    Filtered := False;
    OnFilterRecord := nil;
...
begin
  ...
  begin
    Filtered := False;
    Filter := '';
    OnFilterRecord := OracleClientQueryOrdersFilterRecord;
    Filtered := True;
  end;
  edtExample.Text := '';
end;
...
begin
  statStatus.Panels[0].Text := 'Total Records: ' + VarToStr(OracleClientQueryOrders.Compute('Count(OrderID)', ''));
end;
...
procedure TfrmFilter.OracleClientQueryOrdersFilterRecord(DataSet: TDataSet;
  var Accept: Boolean);
begin
