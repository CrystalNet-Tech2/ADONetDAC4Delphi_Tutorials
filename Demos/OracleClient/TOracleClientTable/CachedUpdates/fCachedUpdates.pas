...
type
  TfrmCachedUpdates = class(TfrmMainQueryBase)
    ...
    OracleClientTableProducts: TOracleClientTable;
...
begin
  OracleClientTableProducts.UpdateBatch;
end;

procedure TfrmCachedUpdates.btnCancelClick(Sender: TObject);
begin
  OracleClientTableProducts.CancelBatch;
end;

procedure TfrmCachedUpdates.btnConnectClick(Sender: TObject);
begin
  inherited;
  OracleClientTableProducts.Close;
  OracleClientTableProducts.Open;
  with OracleClientTableProducts.FieldByName('ProductID') do
    ProviderFlags := [pfInWhere, pfInKey];
end;

procedure TfrmCachedUpdates.btnRevertClick(Sender: TObject);
begin
  OracleClientTableProducts.RevertRecord;
end;

procedure TfrmCachedUpdates.FormCreate(Sender: TObject);
begin
  inherited;
  RegisterDS(OracleClientTableProducts);
end;
