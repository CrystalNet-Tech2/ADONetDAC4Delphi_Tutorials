...
uses
  ...
  StdCtrls, Grids, DBGrids, DBCtrls, ExtCtrls, Buttons, ComCtrls,
...
type
  TfrmCachedUpdates = class(TfrmMainQueryBase)
    ...
    VistaDBQueryEmployee: TVistaDBQuery;
    procedure btnApplyClick(Sender: TObject);
    procedure btnCancelClick(Sender: TObject);
    procedure btnRevertClick(Sender: TObject);
    procedure btnConnectClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure VistaDBQueryEmployeeUpdateRecord(Sender: TObject;
...
begin
  VistaDBQueryEmployee.UpdateBatch;
end;

procedure TfrmCachedUpdates.btnCancelClick(Sender: TObject);
begin
  VistaDBQueryEmployee.CancelBatch;
end;

procedure TfrmCachedUpdates.btnConnectClick(Sender: TObject);
begin
  inherited;
  VistaDBQueryEmployee.Close;
  VistaDBQueryEmployee.Open;
  with VistaDBQueryEmployee.FieldByName('EmployeeID') do
    ProviderFlags := [pfInWhere, pfInKey];
end;

procedure TfrmCachedUpdates.btnRevertClick(Sender: TObject);
begin
  VistaDBQueryEmployee.RevertRecord;
end;

procedure TfrmCachedUpdates.FormCreate(Sender: TObject);
begin
  inherited;
  RegisterDS(VistaDBQueryEmployee);
end;

procedure TfrmCachedUpdates.VistaDBQueryEmployeeUpdateRecord(Sender: TObject;
  EventArgs: TRowUpdatedEventArgs);
var
  oMsg: string;
begin
